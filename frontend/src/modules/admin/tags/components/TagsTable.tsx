  ...
  if (total === 0) {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-xs">
        ...
        <h3 className="text-base font-bold text-slate-700">Chưa có thẻ tag nào</h3>
        <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
          Không tìm thấy dữ liệu thẻ tag phù hợp với điều kiện tìm kiếm hiện tại.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider font-semibold">
              ...
              <th className="py-3.5 px-5">Tên thẻ tag</th>
                ...
                >
                  ...
                  <td className="py-4 px-5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-indigo-100/70 text-indigo-700 flex items-center justify-center font-bold text-sm shrink-0">
                        #
                      </div>
                      <div>
      ...
      <div className="px-5 py-3.5 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
        <div className="flex items-center gap-3">
          <span>
            Hiển thị <strong>{startRecord}</strong> - <strong>{endRecord}</strong> trong tổng số <strong>{total}</strong> thẻ tag
          </span>
